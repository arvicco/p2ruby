...
#include <process.h>
//////////////////////////////////////////////////////////////////////////
#import "libid:{B20A3AED-0B23-4064-B6ED-2CBBE865741C}" no_namespace, named_guids				
//////////////////////////////////////////////////////////////////////////
FILE* g_fLog = NULL;
//////////////////////////////////////////////////////////////////////////
...
public:
	CLog()
	{		
		g_fLog = fopen(".\\P2AddOrderConsole.log", "w");		
...
//////////////////////////////////////////////////////////////////////////
class CConnEvent: 
	public IDispEventImpl<0, CConnEvent, &IID_IP2ConnectionEvent, &LIBID_P2ClientGateMTA, 1>
{
public:
	CConnEvent(wchar_t* appName): m_conn("P2ClientGateMTA.P2ConnectionMTA")			
	{
...
//////////////////////////////////////////////////////////////////////////
class CDSEvents: 
	public IDispEventImpl<0, CDSEvents, &IID_IP2DataStreamEvents, &LIBID_P2ClientGateMTA, 1>
{
public:
	CDSEvents(IP2Connection* conn, wchar_t* name): m_dataStream(L"P2ClientGateMTA.P2DataStreamMTA")
	{	
		DispEventAdvise(m_dataStream);
		m_dataStream->DBConnString = (wstring(L"P2DBSqLite3.dll;;Log\\") + name + L".db").c_str();		
		m_dataStream->type = RT_COMBINED_DYNAMIC;		
		m_dataStream->StreamName = name;
		try
		{
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	string MessagesPath = "p2fortsgate_messages.ini";

	try 
	{				
		if (fopen("P2ClientGate.ini", "r") == NULL)
		{
			fprintf(g_fLog, "P2ClientGate.ini not found ! \n");			
			return 0;
		}
		CConnEvent conn(L"P2FORTS_REPL");		
		CDSEvents dsFUTNFO(conn.GetConn(), L"FORTS_FUTINFO_REPL");		
...
/*		_beginthread(ThreadProc, 0, L"P2MSGSener1");
		...
		IP2BLMessageFactoryPtr msgs("P2ClientGateMTA.P2BLMessageFactoryMTA");
		
		if (fopen(MessagesPath.c_str(), "r") != NULL)
			msgs->Init(MessagesPath.c_str(), L"");
		else
		{
			char str2[50];
			CharToOem("Не найдена схема сообщений !", str2);
			printf(str2);
		}
		...
		while( g_exit == FALSE )				
		{
			...
			{				
				...
				msg->PutField("P2_Type", 1);
				msg->PutField("isin", "RTS-6.10");
				msg->PutField("price", "157750");
				msg->PutField("amount", "1");
				msg->PutField("client_code", "001");
				msg->PutField("type", 1);
				msg->PutField("dir", 1);
				if( ch == '1' ) 
				{
